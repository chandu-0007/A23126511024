...
const TOKEN = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..eZo2nqlfXSWP3w42GAojX4l2L1N-yU-DFd7rYfCkPGc";
...
export async function getAllNotifications(req, res) {
    try {
        ...
        await Log("backend", "error", "handler", `getAllNotifications failed: ${err.message}`);
        res.status(500).json({ error: "Failed to fetch notifications" });
    }
}
export async function getPriorityInbox(req, res) {
    try {
        ...
        await Log("backend", "error", "handler", `getPriorityInbox failed: ${err.message}`);
...
export async function getByType(req, res) {
    try {
        ...
        await Log("backend", "error", "handler", `getByType failed: ${err.message}`);
